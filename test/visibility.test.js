...
describe('Visibility Endpoints', () => {
  ...
  test('PATCH /api/posts/:id/visibility should update post visibility to private with visibility members list and return 200 status code', async () => {
    const response = await request(app)
      .patch(`/api/posts/6617b5fb673289a38b7b7a05/visibility`)
      .send({ visibility: 'private', visibilityMembersList: ['66158127a72308411e9c1e71'] }); // Replace with actual user IDs
    
    expect(response.statusCode).toBe(200);
    expect(response.body.visibility).toBe('private');
    expect(response.body.visibilityMembersList).toEqual(['66158127a72308411e9c1e71']);
  });
  ...
  test('PATCH /api/posts/:id/visibilityList should update visibility members list for a post and return 200 status code', async () => {
    const response = await request(app)
      ...
      .send({ visibilityMembersList: ['66158127a72308411e9c1e71', '66158155a72308411e9c1e79', '66158161a72308411e9c1e7d'] }); // Replace with actual user IDs
    
    expect(response.statusCode).toBe(200);
    expect(response.body.visibilityMembersList).toEqual(['66158127a72308411e9c1e71', '66158155a72308411e9c1e79', '66158161a72308411e9c1e7d']);
  });
